...
if( $row['session_type'] == "CBT" ){
    $cbt_session = array(
        ...
        'post_suds' => $row['postsession_SUDS'],
        'status' => $row['status'],
        'active' => $row['active']
    );
    $_SESSION['cbt_session'] = $cbt_session;
} else if($row['session_type'] == "VRET"){
    $vret_session = array(
        'session_id' => $row['session_id'],
        'therapist_id' => $row['therapist_id'],
        'client_id' => $row['client_id'],
...
<head>
    ...
    <title>DeFear - Session Report</title>
</head>
